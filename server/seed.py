...
def seed_database():
    with app.app_context():
        ...
        admin_profile = Profile(
            user_id=admin.id,
            full_name="Mama Africa Admin",
            region="Headquarters",
            bio="Oversees platform health and safety",
            profile_picture="https://i.pinimg.com/736x/65/1f/b4/651fb4aa0c3bd2b4892e0e23b04f9e0c.jpg"
        )
        db.session.add(admin_profile)
        db.session.commit()
        print("✅ Seeded admin user and profile.")
        ...
        for spec in specialists_data:
            user = User(
                email=spec["email"],
                password_hash=generate_password_hash("123456Ea#"),
                role="health_pro",
                license_number=f"abcd/2025/{random.randint(100000, 999999)}"
            )
            db.session.add(user)
            db.session.flush()
